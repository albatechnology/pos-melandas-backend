    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.banners.update", [$banner->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('bannerable_type') ? 'is-invalid' : '' }}" type="text" name="bannerable_type" id="bannerable_type" value="{{ old('bannerable_type', $banner->bannerable_type) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('bannerable') ? 'is-invalid' : '' }}" type="number" name="bannerable" id="bannerable" value="{{ old('bannerable', $banner->bannerable) }}" step="1" required>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('is_active') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="is_active" id="is_active" value="1" {{ $banner->is_active || old('is_active', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('start_time') ? 'is-invalid' : '' }}" type="text" name="start_time" id="start_time" value="{{ old('start_time', $banner->start_time) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('end_time') ? 'is-invalid' : '' }}" type="text" name="end_time" id="end_time" value="{{ old('end_time', $banner->end_time) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('company') ? 'is-invalid' : '' }}" name="company_id" id="company_id" required>
                    @foreach($companies as $id => $company)
                        <option value="{{ $id }}" {{ (old('company_id') ? old('company_id') : $banner->company->id ?? '') == $id ? 'selected' : '' }}>{{ $company }}</option>
                    @endforeach
                </select>
