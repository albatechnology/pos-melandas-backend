    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.payment-types.update", [$paymentType->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $paymentType->name) }}" required>
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('code') ? 'is-invalid' : '' }}" type="text" name="code" id="code" value="{{ old('code', $paymentType->code) }}">
                @if($errors->has('code'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('payment_category') ? 'is-invalid' : '' }}" name="payment_category_id" id="payment_category_id">
                    @foreach($payment_categories as $id => $payment_category)
                        <option value="{{ $id }}" {{ (old('payment_category_id') ? old('payment_category_id') : $paymentType->payment_category->id ?? '') == $id ? 'selected' : '' }}>{{ $payment_category }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('require_approval') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="require_approval" id="require_approval" value="1" {{ $paymentType->require_approval || old('require_approval', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('company') ? 'is-invalid' : '' }}" name="company_id" id="company_id" required>
                    @foreach($companies as $id => $company)
                        <option value="{{ $id }}" {{ (old('company_id') ? old('company_id') : $paymentType->company->id ?? '') == $id ? 'selected' : '' }}>{{ $company }}</option>
                    @endforeach
                </select>
