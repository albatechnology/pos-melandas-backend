...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            {{ trans('global.edit') }} {{ trans('cruds.colour.title_singular') }}
        </div>

        <div class="card-body">
            <form method="POST" action="{{ route("admin.colours.update", [$colour->id]) }}"
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name"
                           id="name" value="{{ old('name', $colour->name) }}" required>
                    @if($errors->has('name'))
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('code') ? 'is-invalid' : '' }}" type="text" name="code"
                           id="code" value="{{ old('code', $colour->code) }}" required>
                    @if($errors->has('code'))
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('product_brand') ? 'is-invalid' : '' }}"
                        ...
                        @foreach($product_brands as $id => $entry)
                            <option value="{{ $id }}" {{ (old('product_brand_id') ? old('product_brand_id') : $colour->product_brand->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                        @endforeach
                    </select>
