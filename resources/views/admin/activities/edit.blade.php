    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.activities.update", [$activity->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id" required>
                    @foreach($users as $id => $user)
                        <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $activity->user->id ?? '') == $id ? 'selected' : '' }}>{{ $user }}</option>
                    @endforeach
                </select>
                @if($errors->has('user'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('lead') ? 'is-invalid' : '' }}" name="lead_id" id="lead_id" required>
                    @foreach($leads as $id => $lead)
                        <option value="{{ $id }}" {{ (old('lead_id') ? old('lead_id') : $activity->lead->id ?? '') == $id ? 'selected' : '' }}>{{ $lead }}</option>
                    @endforeach
                </select>
                @if($errors->has('lead'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('products') ? 'is-invalid' : '' }}" name="products[]" id="products" multiple>
                    @foreach($products as $id => $products)
                        <option value="{{ $id }}" {{ (in_array($id, old('products', [])) || $activity->products->contains($id)) ? 'selected' : '' }}>{{ $products }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('follow_up_datetime') ? 'is-invalid' : '' }}" type="text" name="follow_up_datetime" id="follow_up_datetime" value="{{ old('follow_up_datetime', $activity->follow_up_datetime) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('follow_up_method') ? 'is-invalid' : '' }}" name="follow_up_method" id="follow_up_method">
                    ...
                    @foreach(App\Models\Activity::FOLLOW_UP_METHOD_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('follow_up_method', $activity->follow_up_method) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('feedback') ? 'is-invalid' : '' }}" name="feedback" id="feedback">{{ old('feedback', $activity->feedback) }}</textarea>
