...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.activityComment.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.activity-comments.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('activity_comment') ? 'is-invalid' : '' }}" name="activity_comment_id" id="activity_comment_id">
                    @foreach($activity_comments as $id => $activity_comment)
                        <option value="{{ $id }}" {{ old('activity_comment_id') == $id ? 'selected' : '' }}>{{ $activity_comment }}</option>
                    @endforeach
                </select>
