...
@can('activity_comment_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            ...
            @include('csvImport.modal', ['model' => 'ActivityComment', 'route' => 'admin.activity-comments.parseCsvImport'])
        </div>
    </div>
@endcan
<div class="card">
    ...
    <div class="card-body">
        <table class=" table table-bordered table-striped table-hover ajaxTable datatable datatable-ActivityComment">
            <thead>
                <tr>
                    <th width="10">
                    ...
                    <td>
                        <select class="search">
                            ...
                            @foreach($activities as $key => $item)
                                <option value="{{ $item->follow_up_datetime }}">{{ $item->follow_up_datetime }}</option>
                            @endforeach
                        </select>
                    </td>
                    <td>
                        <select class="search">
                            <option value>{{ trans('global.all') }}</option>
                            @foreach($activity_comments as $key => $item)
...
@endcan
  ...
  let dtOverrideGlobals = {
    ...
    retrieve: true,
    aaSorting: [],
    ajax: "{{ route('admin.activity-comments.index') }}",
    columns: [
      { data: 'placeholder', name: 'placeholder' },
{ data: 'id', name: 'id' },
{ data: 'content', name: 'content' },
{ data: 'user_name', name: 'user.name' },
{ data: 'activity_follow_up_datetime', name: 'activity.follow_up_datetime' },
{ data: 'activity_comment_content', name: 'activity_comment.content' },
{ data: 'actions', name: '{{ trans('global.actions') }}' }
  ...
  let table = $('.datatable-ActivityComment').DataTable(dtOverrideGlobals);
