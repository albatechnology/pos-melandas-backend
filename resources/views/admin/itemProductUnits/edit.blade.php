...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.itemProductUnit.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.item-product-units.update", [$itemProductUnit->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('product_unit') ? 'is-invalid' : '' }}" name="product_unit_id" id="product_unit_id" required>
                    @foreach($product_units as $id => $product_unit)
                        <option value="{{ $id }}" {{ (old('product_unit_id') ? old('product_unit_id') : $itemProductUnit->product_unit->id ?? '') == $id ? 'selected' : '' }}>{{ $product_unit }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('item') ? 'is-invalid' : '' }}" name="item_id" id="item_id" required>
                    @foreach($items as $id => $item)
                        <option value="{{ $id }}" {{ (old('item_id') ? old('item_id') : $itemProductUnit->item->id ?? '') == $id ? 'selected' : '' }}>{{ $item }}</option>
                    @endforeach
                </select>
                @if($errors->has('item'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('uom') ? 'is-invalid' : '' }}" type="number" name="uom" id="uom" value="{{ old('uom', $itemProductUnit->uom) }}" step="1" required>
                @if($errors->has('uom'))
