...
@can('target_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            ...
            @include('csvImport.modal', ['model' => 'Target', 'route' => 'admin.targets.parseCsvImport'])
        </div>
    </div>
@endcan
<div class="card">
                ...
                <tr>
                    ...
                    <td>
                        <select class="search" strict="true">
                            ...
                            @foreach(App\Models\Target::TYPE_SELECT as $key => $item)
                    ...
                    <td>
                        <select class="search" strict="true">
                            <option value>{{ trans('global.all') }}</option>
                            @foreach(App\Models\Target::SUBJECT_SELECT as $key => $item)
                                <option value="{{ $key }}">{{ $item }}</option>
                            @endforeach
                        </select>
                    </td>
                    <td>
                        <select class="search" strict="true">
                            <option value>{{ trans('global.all') }}</option>
                            @foreach(App\Models\Target::SUBJECT_TYPE_SELECT as $key => $item)
                                <option value="{{ $key }}">{{ $item }}</option>
                            @endforeach
                        </select>
                    </td>
                    <td>
                        <select class="search" strict="true">
                            <option value>{{ trans('global.all') }}</option>
                            @foreach(App\Models\Target::VALUE_TYPE_SELECT as $key => $item)
                                <option value="{{ $key }}">{{ $item }}</option>
                            @endforeach
                        </select>
                    </td>
...
@endcan
  ...
  let dtOverrideGlobals = {
    ...
    retrieve: true,
    aaSorting: [],
    ajax: "{{ route('admin.targets.index') }}",
    columns: [
