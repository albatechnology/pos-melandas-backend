    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.order-trackings.update", [$orderTracking->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('order') ? 'is-invalid' : '' }}" name="order_id" id="order_id">
                    @foreach($orders as $id => $order)
                        <option value="{{ $id }}" {{ (old('order_id') ? old('order_id') : $orderTracking->order->id ?? '') == $id ? 'selected' : '' }}>{{ $order }}</option>
                    @endforeach
                </select>
                @if($errors->has('order'))
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('type') ? 'is-invalid' : '' }}" name="type" id="type">
                    ...
                    @foreach(App\Models\OrderTracking::TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('type', $orderTracking->type) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('type'))
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('context') ? 'is-invalid' : '' }}" name="context" id="context">{{ old('context', $orderTracking->context) }}</textarea>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('old_value') ? 'is-invalid' : '' }}" name="old_value" id="old_value">{{ old('old_value', $orderTracking->old_value) }}</textarea>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('new_value') ? 'is-invalid' : '' }}" name="new_value" id="new_value">{{ old('new_value', $orderTracking->new_value) }}</textarea>
