...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.product-units.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="information">{{ trans('cruds.productUnit.fields.information') }}</label>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.productUnit.fields.information_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="detail">{{ trans('cruds.productUnit.fields.detail') }}</label>
                <input class="form-control {{ $errors->has('detail') ? 'is-invalid' : '' }}" type="text" name="detail" id="detail" value="{{ old('detail', '') }}">
                @if($errors->has('detail'))
                    <span class="text-danger">{{ $errors->first('detail') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.productUnit.fields.detail_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="price">{{ trans('cruds.productUnit.fields.price') }}</label>
                <input class="form-control {{ $errors->has('price') ? 'is-invalid' : '' }}" type="number" name="price" id="price" value="{{ old('price', '') }}" step="0.01">
                @if($errors->has('price'))
                    <span class="text-danger">{{ $errors->first('price') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.productUnit.fields.price_helper') }}</span>
            </div>
            <div class="form-group">
