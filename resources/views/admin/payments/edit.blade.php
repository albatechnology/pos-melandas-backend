    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.payments.update", [$payment->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('payment_type') ? 'is-invalid' : '' }}" name="payment_type_id" id="payment_type_id" required>
                    @foreach($payment_types as $id => $payment_type)
                        <option value="{{ $id }}" {{ (old('payment_type_id') ? old('payment_type_id') : $payment->payment_type->id ?? '') == $id ? 'selected' : '' }}>{{ $payment_type }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('added_by') ? 'is-invalid' : '' }}" name="added_by_id" id="added_by_id">
                    @foreach($added_bies as $id => $added_by)
                        <option value="{{ $id }}" {{ (old('added_by_id') ? old('added_by_id') : $payment->added_by->id ?? '') == $id ? 'selected' : '' }}>{{ $added_by }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('approved_by') ? 'is-invalid' : '' }}" name="approved_by_id" id="approved_by_id">
                    @foreach($approved_bies as $id => $approved_by)
                        <option value="{{ $id }}" {{ (old('approved_by_id') ? old('approved_by_id') : $payment->approved_by->id ?? '') == $id ? 'selected' : '' }}>{{ $approved_by }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('reason') ? 'is-invalid' : '' }}" type="text" name="reason" id="reason" value="{{ old('reason', $payment->reason) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('order') ? 'is-invalid' : '' }}" name="order_id" id="order_id" required>
                    @foreach($orders as $id => $order)
                        <option value="{{ $id }}" {{ (old('order_id') ? old('order_id') : $payment->order->id ?? '') == $id ? 'selected' : '' }}>{{ $order }}</option>
                    @endforeach
                </select>
                @if($errors->has('order'))
