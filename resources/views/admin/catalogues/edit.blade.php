...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.catalogue.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.catalogues.update", [$catalogue->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('channel') ? 'is-invalid' : '' }}" name="channel_id" id="channel_id">
                    @foreach($channels as $id => $channel)
                        <option value="{{ $id }}" {{ (old('channel_id') ? old('channel_id') : $catalogue->channel->id ?? '') == $id ? 'selected' : '' }}>{{ $channel }}</option>
                    @endforeach
                </select>
