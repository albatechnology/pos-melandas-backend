        ...
        <form method="POST" action="{{ route("admin.leads.update", [$lead->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('type') ? 'is-invalid' : '' }}" name="type" id="type" required>
                    ...
                    @foreach(App\Enums\LeadType::getInstances() as $enum)
                        <option value="{{ $enum->value }}" {{ old('type', $lead->type->value) === (string) $enum->value ? 'selected' : '' }}>{{ $enum->label }}</option>
                    @endforeach
                </select>
                @if($errors->has('type'))
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status" required>
                    ...
                    @foreach(App\Enums\LeadStatus::getInstances() as $enum)
                        <option value="{{ $enum->value }}" {{ old('status', $lead->status->value) === (string) $enum->value ? 'selected' : '' }}>{{ $enum->label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('is_new_customer') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="is_new_customer" id="is_new_customer" value="1" {{ $lead->is_new_customer || old('is_new_customer', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('label') ? 'is-invalid' : '' }}" type="text" name="label" id="label" value="{{ old('label', $lead->label) }}">
                @if($errors->has('label'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('customer') ? 'is-invalid' : '' }}" name="customer_id" id="customer_id">
                    @foreach($customers as $id => $customer)
                        <option value="{{ $id }}" {{ (old('customer_id') ? old('customer_id') : $lead->customer->id ?? '') == $id ? 'selected' : '' }}>{{ $customer }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('channel') ? 'is-invalid' : '' }}" name="channel_id" id="channel_id" required>
                    @foreach($channels as $id => $channel)
                        <option value="{{ $id }}" {{ (old('channel_id') ? old('channel_id') : $lead->channel->id ?? '') == $id ? 'selected' : '' }}>{{ $channel }}</option>
                    @endforeach
                </select>
